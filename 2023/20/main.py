...
for p2 in (False, True):
	...
	for _ in range(1000000000 if p2 else 1000):
		...
		while q:
			...
			if kind == "conjunction":
				...
				if p2 and name == "lb": # XXX a bit hardcoded whatever
					for k, v in mem.items():
						if v:
							dep_presses[k] = presses

					if len(dep_presses) == len(ins):
						print(math.lcm(*dep_presses.values()))
						exit()
